...
$(document).ready(function() {
    ...
    $(function() {
        ...
        function start_send_web_vitals(el, ajax_action, params) {
            ...
            let method='open';
            if ($(el.target).is('form')) {
                method='submit_attempt'; 
            }
            if ($('.action-redirect',$('#popup-login-wrap .popup-content')).is('.action-redirect')) {
                method='submit'; 
            } 
            if ($('#registerform',$('#popup-login-wrap .popup-content')).is('#registerform') && (params['success']??false)) {
                ga4.send('popup_tracking','open', 'signup_form', 'hybrid');
            }else if (ajax_action === 'registration') {
                ga4.send('popup_tracking',method, 'signup_form', 'hybrid');
            } else if (ajax_action === 'login') {
                if ($('[name=vwlp_start]', el.target).val()==='start' && (params['success']??false)) {
        ...
        function reLoad(element) {
            ...
            if ($("[href*='#login_popup']", element).is('a')) {
                $("[href*='#login_popup']", element).click(function (e) { ga4.send('popup_tracking', 'click', 'login_form', 'hybrid'); return PopupLoginClick(e, 'login'); });
            }

            if ($(location).attr('href').indexOf('login_popup')!==-1) {
                $($("[href*='#login_popup']", element).eq(0)).click();
            }
    ...
    if ($("#btn-login-popub").is('#btn-login-popub')) {
        $(document).on('click', '#popup-login-wrap .login_form_social a', function (event) {   
            event.preventDefault();   
            var startLink=false;  
            var link=event.currentTarget.href;  
            const evGa4={
                'event': 'ga4_interaction',
                'action': 'popup_tracking',
                'parameters': {
                  "role": "ux",
                  "method": 'submit_attempt',
                  "place": 'login_form',
            ...
            (!!logger)&&logger.console('.login_form_social a::click', 'submit_attempt', evGa4);
        }); 
    }
